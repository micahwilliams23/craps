...
# play_6_rounds <- function(){
games = 10000
contest_size = 6
n <- games * contest_size


outcomes <- tibble(p_IB = map_int(1:n, ~ roll_two_dice()),
                p_JM = map_int(1:n, ~ roll_two_dice())) %>%
  mutate(IB_craps = p_IB %in% c(7,11),
         JM_craps = p_JM %in% c(7,11)) %>%
  mutate(winner = ifelse(IB_craps == JM_craps, 'Tie',
                  ifelse(IB_craps == TRUE, 'IB', 'JM')),
         contest = rep(1:games, each = contest_size)) %>%
  group_by(contest) %>%
  summarize(JM_wins = sum(winner == 'JM'),
            IB_wins = sum(winner == 'IB'))

ggplot(outcomes, aes(JM_wins)) +
